...
public class DicUtils {
     ...
     */
    private static Map<String, Map<Byte, Dic>> caches = new ConcurrentHashMap<>();

    /**
     * 获取字典类的所有值的集合
     */
    public static Map<Byte, Dic> getItems(final Class<?> dicClass) {
        if (!dicClass.isEnum()) {
        ...
        // 先从缓存中查找
        Map<Byte, Dic> items = caches.get(dicClass.getName());
        if (items != null) {
            return items;
        }
     ...
     * @param code     字典的编码
     */
    public static Dic getItem(final Class<?> dicClass, final byte code) {
     ...
     * @param code     字典的编码
     */
    public static boolean isValid(final Class<?> dicClass, final byte code) {
